...
public class LivenessPlugin extends CordovaPlugin implements NecFlowHandler {
    ...
    private void initialize() {
        ...
        Log.println(Log.ASSERT, "ASSETS", cordova.getActivity().getAssets().list(""));
    ...
    @Override
    public void onError(@NonNull NecFaceLivenessError error, NecFlowData data) {
    ...
    @Override
    public void onInit(@Nullable NecFaceLivenessError necFaceLivenessError) {
        cordova.getActivity().runOnUiThread(() -> {
            if (necFaceLivenessError == null)
                callback.success();
            else
                callback.error(String.valueOf(necFaceLivenessError));
            callback = null;
        });
    }
}